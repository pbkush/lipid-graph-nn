...
- `MembranePropertyGNN.forward` ([membrane_prop_gnn.py](../lipid_gnn/membrane_prop_gnn.py)): `x_dict, edge_index_dict, batch_dict, edge_attr_dict` → pooled `graph_repr` → MLP head. Per-bead embeddings `bead_h` are accessible inside the forward, so node-level heads are cheap.
- Positions are **not** stored on graphs; the chunked `.pt` files carry `data['bead'].x` [N,4], bonded `edge_attr` [B,2], spatial `edge_attr` [S,16] (RBF only — **raw distance is discarded**), `y` [1,num_props]. No system id, no frame index.
...
7. Create `lipid_gnn/representation_gnn.py`. Encoder mirrors `MembranePropertyGNN`'s HeteroConv+GATv2+GraphNorm stack (initially duplicated — shared factoring is a later cleanup once both tracks are green). Two heads: `force_head: MLP(hidden_dim → 3)` on per-bead `bead_h`, `order_head: MLP(hidden_dim → K_order)` on per-lipid mean-pooled `bead_h` grouped by `bead_to_lipid`. No supervised property head. No composition input — the class is intentionally composition-blind.
...
- Composition blindness: `RepresentationGNN` has no composition input by construction.
