...
| 10 | HPC benchmark (`benchmark_hpc.sh` + `analyze_benchmark.py`); populate `hpc_defaults` | [x] | 10-point sweep landed 2026-05-16. Winner `8_sim_8gpu_cpu4`: 41,909 ns/day aggregate (~5,240 ns/day per slot). `hpc_defaults`: sims_per_node=8, gpus_per_node=8, cpus_per_sim=4, mem_per_sim=16G. **Recommended walltime for 1 µs: 8 h** (raw 4.6 h × ~1.7× safety margin for late-run slowdown). |
...
- **Decision 63 (2026-05-16)** — GPU `hpc_defaults` locked from the 10-point benchmark: `sims_per_node=8, gpus_per_node=8, cpus_per_sim=4, mem_per_sim=16G`. Winner `8_sim_8gpu_cpu4` at 41,909 ns/day aggregate (score 8.6 M ns·day/node·hour). Notable: `8_sim_8gpu_cpu8` is **−15 %** despite 2× CPUs (MI210 GPUs are not CPU-thread limited at this system size); `16_sim_8gpu_share` aggregates higher (53,730) but per-slot drops to 3,358 ns/day (~36 %), and the 2× checkpoint/IO cost would erode the throughput gain for production runs — not worth the operational complexity.
- **Decision 64 (2026-05-16)** — Recommended SLURM `--time` for 1 µs production on GPU = **8 h**. Per-slot rate at the locked config is ~5,240 ns/day → 4.6 h raw; ×1.7 margin covers PME re-tuning, occasional node sharing, late-run DLB jitter. Resubmit-with-`-cpi` still available if any slot overshoots.
