...
def save_results(results, filepath='results.h5'):
    with open(filepath, 'wb') as file:
        pickle.dump(results, file, protocol=pickle.HIGHEST_PROTOCOL)

def load_results(filepath='results.h5'):
    try:
        ...
        while True:
            if process.returncode is not None:  # Process has finished
                if process.returncode != 0:
                    write("Process exited with an error {process.returncode}.")
                    raise
                else:
            ...
            if condition_check():
                ...
                write(f'******************* {parent_proc.is_running()}')
...
def wait_flag(flag='proceed.txt'):
    while True:
...
def initialize_plot():
    ...
    plt.subplots_adjust(left=0.17, bottom=0.22, right=0.99, top=0.98)
    return figure, ax
...
def initialize_equilibrium_simulations(initial_path, nA, nB):
    ...
    if not os.path.exists('A.trr'):
        remove('A.trr')
        remove('B.trr')
        if initial_path.final_states[0] == 'A':
            initial_path.frames([-2,-1]).write('A.trr',
                invert_velocities=False, reset_time=True)
            initial_path.frames([+1,+0]).write('B.trr',
                invert_velocities=True, reset_time=True)
        else:
            initial_path.frames([-2,-1]).write('B.trr',
                invert_velocities=False, reset_time=True)
            initial_path.frames([+1,+0]).write('A.trr',
                invert_velocities=True, reset_time=True)
    for i in range(nA):
        path = f'equilibriumA/traj{i + 1:06g}.part0000.trr'
        if not os.path.exists(path):
            shutil.copy('A.trr', path)
            write(f'    {path}')
    for i in range(nB):
        path = f'equilibriumB/traj{i + 1:06g}.part0000.trr'
        if not os.path.exists(path):
            shutil.copy('B.trr', path)
            write(f'    {path}')
    ...
    for k, chain in enumerate(shots.pathensembles):
        ...
        if 'pool.h5' in os.listdir(f'{directory}/shots{k}'):
            try:
                ...
                write('warning: reloading pool_backup.h5 instead of pool.h5')
        ...
        keepers = np.random.choice(
            len(pool), min(len(pool), selection_pool_size), replace=False)
        pool = pool[keepers]
...
def load_initial_path(states_function, descriptors_function, values_function,
    ...
    initial_path = PathEnsemble(directory, '../run.gro',
            states_function, descriptors_function, values_function)
    initial_path.append('initial.trr')
    initial_path.split()
...
def results_available(folder='.'):
    ...
    return ['result.trr' in os.listdir(f'{folder}/{child}')
...
def run_acceptance_rejection_on_latest_path(chain, bins, densities):
    ...
    if chain.are_accepted[-1] and chain.are_transitions[-1]:
        if leading is not None:
            update_values(chain, only_zeros=True)  # get the values
            keepers = [leading, -1]
...
def update_selection_pool(pool, chain, pool_index,
    ...
    # remove initial_path in pool
    if ('initial.trr' in pool.shooting_trajectory_filenames[0]
        and len(pool) > 1):
        if (not at_least_one_transition) or np.sum(pool.are_transitions) > 1:
            pool = pool[1:]
            write(f'    removed the initial path in pool')
...
def get_bins(pathensemble, n_bins,
    ...
    # restrict cutoff_max to shots range
    if len(shots):
        values = np.append([v[+1] for v in shots.values(
                            np.where((shots.initial_states == 'A') *
                                      shots.are_accepted)[0])
                            if len(v) >= 3],
                           [v[-2] for v in shots.values(
                            np.where((shots.final_states == 'A') *
                                      shots.are_accepted)[0])
                            if len(v) >= 3])
    else:
        values = np.zeros(0)
    if len(values):
        begin = np.clip(np.median(values), -cutoff_max, -cutoff_min)
    else:
        begin = -cutoff_min
    if len(shots):
        values = np.append([v[+1] for v in shots.values(
                            np.where((shots.initial_states == 'B') *
                                      shots.are_accepted)[0])
                            if len(v) >= 3],
                           [v[-2] for v in shots.values(
                            np.where((shots.final_states == 'B') *
                                      shots.are_accepted)[0])
                            if len(v) >= 3])
    else:
        values = np.zeros(0)
    if len(values):
        end = np.clip(np.median(values), +cutoff_min, +cutoff_max)
    else:
        end = +cutoff_min
...
def select_shooting_point(pathensemble, pool, bins, densities,
    ...
    if t0 is not None:
        keepers = (equilibrium.frame_states == 'R') * (
                   equilibrium.frame_simulation_times >= t0)
    else:
        keepers = np.zeros(equilibrium.nframes, dtype=bool)
    eq_size = np.sum(keepers)
    if eq_size:
    ...
    indices = np.random.choice(len(pool), eq_size)  # fake pool indices
    write(f'equilibrium data yielded {eq_size} candidate points')
    write(f'    hist: {histogram}')
    
    # try selecting from equilibrium: select putative bin
    weights = correction[1:-1].copy()
    weights /= np.sum(weights)
    k = np.random.choice(len(bins) - 1, p=weights)
    write(f'    trying to override with bin {k}')
    eq_candidates = np.where(bin_indices == k)[0]
    if len(eq_candidates):  # are there equilibrium points in putative bin?
        write(f'    success')
        i = np.random.choice(eq_candidates)
        write(f'selecting shooting point {files[i]}, {positions[i]}')
        fname = pool[indices[i]].trajectory_files[0]
        write(f'   pool position: {indices[i]} ({fname})')
        k = np.digitize(values[i], bins)-1
        write(f'   value: {values[i]:.2f}, bin {k}, state: {states[i]}')
        selection_bias = correction[k + 1]
        
        # update time
        frame_simulation_times = equilibrium.frame_simulation_times
        if len(frame_simulation_times):
            new_t0 = np.max(frame_simulation_times)
    
    else:  # selecting from pool
    ...
    try:
        return (extract_frame(files[i], positions[i], '../run.gro'),
                selection_bias, indices[i], new_t0)
    except:
...
def plot_2d_energy(X, Y, F, levels,
    ...
    plt.gca().set_aspect(abs((X[-1, -1] - X[0, 0]) / (Y[-1, -1] - Y[0, 0])))
    plt.gca().set_position((-.05,0.18,.8,.8))
