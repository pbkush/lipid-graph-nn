...
- **Serial sweep variant for non-gres GPU nodes (2026-05-21)**: [scripts/bash/submit_sweep_serial.sh](../../scripts/bash/submit_sweep_serial.sh) + [scripts/bash/sbatch_sweep_serial.sh](../../scripts/bash/sbatch_sweep_serial.sh). Companion to the packed parallel sweep — submits one sbatch per HP cell × seed-group, runs `run_sweep.py` once per job, and emits **no `--gres` flag** by default so jobs schedule on Goethe GPU nodes that allocate whole nodes instead of single-GPU slices. Each `--seeds "0 1 2"` group becomes one job that iterates those seeds sequentially via `SWEEP_SEEDS` (config-default `num_workers=6`, `pin_memory=True` — no IPC contention because there is no per-node fan-out). New flag `--chunks-dir <abs-path>` forwards `CHUNKS_DIR` via `--export` so different label sets (e.g. `prop_legacy_bugged_random` vs `prop_legacy_bugfixed_s0`) can be selected per command without editing config. Opt back in to `--gres "gpu:1"` for gres-enabled partitions. Use when packed `--gres=gpu:N` nodes are unavailable, or when you want closer parity with pre-2026-05 single-job-per-cell behaviour.
- **HPC chunks path layout fix (2026-05-21)**: both `sbatch_sweep.sh` and `sbatch_sweep_serial.sh` previously derived `WORK` by appending `preprocessed_graphs/active` (the local swap-able symlink layout). On HPC the train/val/test split dirs live flat under `preprocessed_graphs/` (no `active` segment), so the old derivation pointed one level too deep. Both scripts now hard-code `WORK=/work/${GROUP_HPC}/${USER}/${WORK_SUBPATH}/preprocessed_graphs` and the serial sbatch honours an externally-supplied `CHUNKS_DIR` (`${CHUNKS_DIR:-$WORK}`) so label-set subdirs can be selected per-job.
