...
| 11d — CHOL-containing cells across 3a/3b grids | **pending** | CHOL partner of `popc_interpolation` + `dppc_corner` + `dopc_corner` (capped 40 % per Decision 35) still to run; resubmit with the same three `--missing-from-grid` calls — the CSV gate will skip everything already done |
