...
- **Marimo analysis notebooks**: Confirmation-stage analyses use reactive marimo `.py` notebooks (e.g. `scripts/notebooks/analyze_stage_5.py`). Written following the `marimo-data-analysis` skill: all imports in cell 1, prose-first, figures returned as `_fig,` (not `plt.show()`), `mo.stop` guards after data loads, `mo.callout` for key findings. Train-composition discovery uses `CONFIG.paths.data_dir` listing rather than `torch.load` on chunk files (avoids `torch_geometric` dep in fresh `uv` envs). Verify with `uvx marimo check <file>` + `uv run <file>` before declaring done.
- **Stratified preprocessing**: `preprocess_graphs.py` defaults to `--split-method stratified`. To guarantee proper y-range coverage per split for the upcoming experiment's properties, pass `--stratify-on lipid_packing thickness variation thickness_std` (Tier A). Stratifies by k-means in z-scored y-space; prints per-split y-stats to verify. The `random` fallback is available for reproducibility tests. Required `--props-set` picks the property subfolder under `results/properties/` (e.g. `prop_legacy_bugfixed_s0`); outputs land at `data/preprocessed_graphs/<run-name>/{train,val,test}/` with archive at `data/preprocessed_graphs/archives/<run-name>.zip` (`<run-name>` defaults to `--props-set`).
